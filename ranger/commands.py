...
class fzf_select(Command):
    ...
    def execute(self):
        ...
        env['FZF_DEFAULT_OPTS'] = '--layout=reverse --ansi --preview="{}" --preview-window=right,40%'.format('''
            (
                batcat --color=always {} ||
                bat --color=always {} ||
                cat {} ||
                eza --tree --icons=always --level=1 --color=always {} ||
    ...
    def execute(self):
        ...
        if rg is not None and fzf is not None and bat is not None and editor is not None:
            ...
            if (self.fm.thisdir.path == self.fm.home_path):
                self.fm.notify(
                    "Searching from home directory is not allowed", bad=True)
                return
            ...
            if fzf.returncode == 0:
                ...
                self.fm.execute_command(
                    editor + " " + selected_line + " " + file_basename)
...
class fd_search(Command):
    ...
    def execute(self):
        ...
        self.fm.notify("Executing command: {}".format(command))
